...
namespace Scenarios.StackOverflow
{
  ...
  public class GetStackOverflowQuestions : IActivity
  {
    ...
    private ILogger _logger = Log.ForContext<GetStackOverflowQuestions>();
    ...
    public async Task RunAsync(CancellationToken ct)
    {
      ...
      var decompressingClientHandler = new HttpClientHandler { AutomaticDecompression = DecompressionMethods.GZip };
      ...
      using (httpClient)
      {
        for (var pageIndex = 1; pageIndex <= numberOfPages; pageIndex++)
        {
          ...
          {
            _logger.Information("Preparing to request page {pageIndex} StackOverflow"); // implicit use property from context/scope.

            HttpResponseMessage response;
            using (_logger.TimeOperation("Querying StackExchange API"))
            {
              var requestUrl = $"questions?page={pageIndex}&pagesize={pageSize}&order=desc&sort=creation&site=stackoverflow";
              response = await httpClient.GetAsync(requestUrl, ct);
              _logger.Debug("Request to {requestUri} return response with status code {statusCode}", requestUrl, response.StatusCode);
            }

            if (!response.IsSuccessStatusCode)
            {
              _logger.Warning("The request to StackExchange API was unsuccessful. Status code: {statusCode}, Reason {failReason}", response.StatusCode, response.ReasonPhrase);
            }
            ...
            using (var jsonReader = new JsonTextReader(textReader))
            {
              try
              {
                ...
                allQuestions.AddRange(questions.Items);
